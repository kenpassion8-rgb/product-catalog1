...
### 4.1 Base64 嵌入（必须）
- 所有产品图片**必须 base64 嵌入**到 HTML 文件中
- 不使用任何外链（imgur、CDN 等），因为外链可能失效
...
7. 输出完整 HTML 文件，文件名建议：`index.html`（用于部署到 Vercel）

### 情况 B：同品类，新增产品

1. 打开已有的 HTML 文件
2. 确认当前最大编号（查看文件中最后一个 BM 编号）
3. 用户上传新产品图片
4. 从最大编号 +1 开始分配
5. 在产品网格末尾追加新卡片
6. 输出完整更新后的 HTML 文件
...
### 必须做的
- ✅ 所有图片 base64 嵌入，不使用外链
- ✅ 产品卡片只显示编号（BM001 等），不显示名称或克重
- ✅ 图片统一使用 `object-fit: contain` + `padding: 16px`
- ✅ 固定4列网格（电脑版），响应式缩减至3列/2列
- ✅ 输出完整 HTML 文件（不是代码片段）
- ✅ 每次输出的文件名为 `index.html`
